	...
	app.put('/api/giahanluutru', function(req, res, next){
	    ...
	    try{
	    	student.findOne({_id: idStu}, function(err, stuRes){
	    		if(err) return next(err);
	    		stuRes.update({ $set: {_hoc_luc_id: svhocluc, _hoan_canh_id: svhoancanh, _phong_id: svloaiphong, gia_han_luu_tru: true, da_dong_tien: false, xet_duyet_thanh_cong: false} }, function(err1, re){
